...
def _einst(**kw) -> Einstellungen:
    return Einstellungen(**{"aktiv": True, "abstand_sekunden": 300, **kw})
...
def test_ohne_laufenden_prozess_wird_gestartet(sperre: Sperre) -> None:
    ...
    assert starter.befehle[0][1:] == ["-m", "fbgroups.cli", "campaign", "automatik"]
...
def test_jeder_blick_wird_gemeldet(sperre: Sperre) -> None:
    """**Punkt 11.** Drei unterscheidbare Arten, jede mit Grund im Klartext."""
    ...
    assert gemeldet[0].art in {"laeuft", "gestartet", "abgeschaltet"}
