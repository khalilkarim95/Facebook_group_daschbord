...
def _fern_client(bestand: Path):
    ...
    return TestClient(
        create_app(config=Konfig(bestand), db_path=bestand),
        headers={"Origin": "http://127.0.0.1:8090"},
    )


def _hole_schritt(client, bestand: Path) -> dict:
    """Wie ``fuehre_lauf_fern_aus``: nachfragen, bei ``warten`` die Zeit vorstellen."""
    for _ in range(20):
        daten = client.post("/automatik/naechster", json={}).json()
        if daten.get("schritt") is not None:
            return daten["schritt"]
        if daten.get("warten"):
            _vorspulen(bestand, int(daten["warten"]))
            continue
        assert daten.get("weiter"), f"kein Schritt, kein Warten, kein weiter: {daten}"
    raise AssertionError("es kam kein Schritt")


def test_im_fernbetrieb_elf_gruppen_zwei_runden(bestand: Path) -> None:
    """Derselbe Kern ueber den Server - dort lag die Schleife aus dem Log.

    g01 meldet sich wie ein **aelterer** Arbeitsrechner (``erschoepft``);
    auch das haelt die Runde nicht auf und macht die Gruppe nicht fertig.
    """
    client = _fern_client(bestand)
    besucht: list[str] = []
    runden: list[tuple[int, int, int]] = []

    for _ in range(22):
        s = _hole_schritt(client, bestand)
        besucht.append(s["group_id"])
        runden.append((s["runde"], s["runde_platz"], s["runde_gruppen"]))
        ergebnis = _ausgang(s["group_id"], len(besucht))
        meldung = {
        ...
        if s["group_id"] == "g01":
            meldung["kein_anlass"] = False
            meldung["erschoepft"] = True
        assert client.post("/automatik/ergebnis", json=meldung).status_code == 200
        _vorspulen(bestand, SCHRITT_SEKUNDEN)

    assert besucht[:11] == GRUPPEN
    assert besucht[11:] == GRUPPEN
    assert runden[0] == (1, 1, 11)
    assert runden[10] == (1, 11, 11)
    assert runden[11] == (2, 1, 11), "Runde 2 beginnt wieder bei der ersten"
    with MarketingStore(bestand) as store:
        assert store.erschoepfte_gruppen(KAMPAGNE) == {}
